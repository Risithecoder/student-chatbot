...
/**
 * Recommend mocks based on weak topics from pre-computed stats.
 * Returns MOCK_LINK tag strings that the renderer can parse.
 * @param {Object} stats - Pre-computed stats from stats-engine
 * @param {number} maxResults - Maximum number of recommendations
 * @returns {string} - MOCK_LINK tags as a string block
 */
export function getRecommendedMocks(stats, maxResults = 2) {
  // Collect weak topics (accuracy < 60%)
  const weakTopics = [];

  const ta = stats.topic_analysis || {};
  for (const [topic, data] of Object.entries(ta)) {
    const acc = typeof data.accuracy === 'number' ? data.accuracy : 0;
    if (acc < 60) weakTopics.push({ topic, accuracy: acc });
  }

  const sa = stats.section_analysis || {};
  for (const [section, data] of Object.entries(sa)) {
    const acc = typeof data.avg_accuracy === 'number' ? data.avg_accuracy : null;
    if (acc !== null && acc < 60) weakTopics.push({ topic: section, accuracy: acc });
  }
  ...
  const weakNames = weakTopics.map(w => w.topic.toLowerCase());

  // Score each mock by how many weak topics it covers
  const scored = MOCK_CATALOG.map(mock => {
    let matchCount = 0;
    let matchedTopics = [];
    for (const t of mock.topics) {
      if (weakNames.some(w => t.toLowerCase().includes(w) || w.includes(t.toLowerCase()))) {
        matchCount++;
        matchedTopics.push(t);
      }
    }
    return { mock, matchCount, matchedTopics };
  })
  ...
  if (scored.length === 0) {
    // No relevant mocks found — don't recommend irrelevant ones
    return '';
  }
  ...
  return scored
    .map(s => {
      const weakInfo = weakTopics
        .filter(w => s.matchedTopics.some(t => t.toLowerCase().includes(w.topic.toLowerCase())))
        .map(w => `${w.topic} at ${w.accuracy}%`)
        .join(', ');
      const reason = weakInfo
        ? `Your weak areas: ${weakInfo} — this mock targets them directly`
        : `Recommended to strengthen ${s.matchedTopics.join(', ')}`;
      return buildMockLinkTag(s.mock, reason);
    })
    .join('\n');
}

function buildMockLinkTag(mock, reason) {
  return `<MOCK_LINK id="${mock.id}" name="${mock.name}" difficulty="${mock.difficulty}" questions="${mock.questions_count}" topics="${mock.topics.join(', ')}" reason="${reason}"/>`;
}
